...
def build_lut(file_list, lower_left, srs):
    ...
    zoom_levels.sort()
    print("zoom_level = %s"%zoom_levels)
    matrix = []
    ...
    for zoom in zoom_levels:
        ...
        if zoom - 1 in [item for item in zoom_levels if item == (zoom - 1)]:
            ...
            x_width_max = max([int(item['x']) for item in file_list if int(item['z']) == level.zoom])
            x_width_min = min([int(item['x']) for item in file_list if int(item['z']) == level.zoom])
            level.matrix_width = (x_width_max - x_width_min) + 1
            y_height_max = max([int(item['y']) for item in file_list if int(item['z']) == level.zoom])
            y_height_min = min([int(item['y']) for item in file_list if int(item['z']) == level.zoom])
